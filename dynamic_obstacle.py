...
class Obstacle():
    ...
    def update_loc(self, t):
        t_duration = t - self.t
        loc = (self.acc* t_duration * t_duration)
             + (self.vel[1] * t_duration)
             + (self.pos[1])
        return loc
...
class ObstacleField(object):
    ...
    def random_init_obstacle(self, t, vehicle_x = 0, vehicle_y = -1, min_dist = 0.1):
        ...
        x = y = -1
        while (dist < min_dist):
            pos = random.uniform(FIELD_X_BOUNDS[0],FIELD_X_BOUNDS[1], 2)
            pos_vehicle = [vehicle_x, vehicle_y]
            dist = np.linalg.norm(pos-pos_vehicle)
            #TODO (distance to car): add a dist from static obs
        vel = random.uniform(1e-3, 1e-2, 2) * random.choice([1,-1])
        acc = random.uniform(5e-6, 1e-4, 2) * random.choice([1,-1])
